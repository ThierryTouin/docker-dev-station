#!/bin/bash
    ...
    _dcmd_completions()
    {
        ...
        optsNiv1=" portainer glowroot chrome mariadb mongo-db mysql postgresql liquibase omnidb drupal liferay liferay-full dds-strapi elastic1 elastic2 keycloak ldap ldap-admin java fake-smtp dds_mockmock glances logs apache kong traefik1 traefik2 n8n dufs file-manager filegator sonar-cli sonar S3 dnsmasq fossflow it-tools mermaid networking-scanopy networking-toolbox pdf vscode web-terminal ui"                  # Les options disponibles pour autocomplétion
